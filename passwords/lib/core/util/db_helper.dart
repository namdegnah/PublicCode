...
class DBHelper {
  ...
  static Future<List<Map<String, dynamic>>> getDataOrderById(String table) async {
    try{
      final db = await database();
      return await db.rawQuery('SELECT * FROM $table ORDER BY id');
    } catch (error){
      throw Exception('could not retrieve data from $table : $error');
    }
  }
